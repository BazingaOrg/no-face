...
**Wheel/pinch zoom**: the same active face's emoji can also be scaled directly on the canvas, through the same `onRepositionActiveEmoji`/`scheduleReposition` path (so it's `isCustom: true` and mirrors the inspector's slider live). Desktop: hovering the active emoji's screen rect and scrolling adjusts scale ~5% per notch, clamped to `[0.5, 2.0]` (matching the inspector slider). This is wired as a native `addEventListener('wheel', ..., { passive: false })` in a `useEffect`, not the JSX `onWheel` prop — React registers synthetic wheel/touch listeners as passive by default, so `preventDefault()` inside `onWheel` is silently ignored and won't stop page scroll. Mobile: a second finger landing while a drag is active cancels the drag (releases its pointer capture) and starts a pinch, scaling by the ratio of current to initial two-finger distance. Both gestures call `onBeginDragReposition` once per gesture (wheel: coalesced by a 400ms idle gap between notches; pinch: once when the second pointer lands) rather than once per frame, so undo steps stay coarse.
